...
External pull requests are not included in the triage request queue.
...
- **Frontier query**: list the map's open children, drop any with an open `Blocked by` issue or the `wayfinder:claimed` label; first in map order wins.
